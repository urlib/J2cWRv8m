...
#
#	$Id: uc.php 25 2014-01-10 03:13:43Z along $
#
...
class uc_note {
	...
	function synlogin($get, $post) {
		...
		if($member = $this->db->fetch_array($query)) {
			...
			pd_setcookie('phpdisk_info',pd_encode("$userid\t$gid\t$username\t$password\t$email"),$cookietime);
		} else {
			...
			pd_setcookie('phpdisk_info',pd_encode("$data[0]\t$gid\t$data[1]\t$password\t$data[2]"),$cookietime);
		}
	}
	function synlogout($get, $post) {
		...
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		pd_setcookie('phpdisk_info', '', -86400 * 365);
	}
...
function pd_encode($txt, $operation = 'ENCODE',$key = ''){
	global $settings;
	$key = $key ? $key : ($settings['encrypt_key'] ? $settings['encrypt_key'] : 'PHPDisk=Rc9o');
	if($operation == 'DECODE'){
