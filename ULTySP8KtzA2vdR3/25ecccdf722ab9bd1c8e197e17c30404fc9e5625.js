var bt = 
{
	...
	render_form_line:function(item,bs){
		...
		if(!bs) bs = '';
		var _title = '';
		if(item.title) _title = "<span class='tname'>"+item.title+"</span>"
		
		var _html = "<div class='info-r'>";
		var _name = item.name;
		var _placeholder = item.placeholder;
				...
				for(var x=0;x<item.items.length;x++){
					...
					switch(_obj.type){
						...
						case 'textarea':
							...
							_html += '<textarea class="bt-input-text '+ _name + bs+'" name="'+_name+'" style="width:'+_width+';height:100px;line-height:22px"></textarea>';	
							if(_placeholder) _html+='<div class="placeholder c9" style="top: 10px; left: 10px; display: block;">'+_placeholder+'</div>';							
							break;
						case 'button':
							var _width = _obj.width?_obj.width:'330px';
			...
			_html += '</div>';
			return { html: _title + _html,clicks:clicks,data:item};			
		},
		render_form:function(data,callback){
			if(data){		
			var bs = '_' + bt.get_random(6);
			var _form = $("<div data-id="+bs+" class='bt-form ptb15  pb70'></div>");			
			...
			for (var i = 0;i<_lines.length;i++){
				...
				var d = '';
				if(_lines[i].hide) d = 'style="display:none;"'
				_form.append("<div "+d+" class='line'>" + rRet.html + "</div>");				
			}
	...
	rec_install:function(){		
		bt.send('GetSoftList','ajax/GetSoftList',{},function(l){
			...
			var k = layer.open({
				...
				closeBtn: 2,
				shadeClose: false,
				content: "<div class='rec-install'><div class='important-title'><p><span class='glyphicon glyphicon-alert' style='color: #f39c12; margin-right: 10px;'></span>"+lan.bt.install_ps+" <a href='javascript:jump()' style='color:#20a53a'>"+lan.bt.install_s+"</a> "+lan.bt.install_s1+"</p></div><div class='rec-box'><h3>"+lan.bt.install_lnmp+"</h3><div class='rec-box-con rec-box-con-hover'><ul class='rec-list'>" + c + "</ul><div class='onekey'>"+lan.bt.install_key+"</div></div></div><div class='rec-box' style='margin-left:16px'><h3>LAMP</h3><div class='rec-box-con'><ul class='rec-list'>" + g + "</ul><div class='onekey'>一键安装</div></div></div></div>"
			});
			...
			$(".onekey").click(function() {
				...
				function install_plugin(q){
					...
					s = "sName=" + p + "&version=" + x + "&type=0&id=" + (t + 1);					
...
bt.ftp = {	
	...
	{
		bt.render_form(bt.data.ftp.add,function(rdata){		
			if(callback) callback(rdata);
		});		
	},
...
bt.system = {	
	...
	rep_panel:function(callback){
		...
		bt.send('RepPanel','ajax/RepPanel',{},function(rdata){
			loading.close();
			if(callback) callback(rdata);
		})
	},
...
bt.soft = {
			...
			bt.soft.pub.wxpayTimeId = setInterval(function(){
				bt.soft.pro.get_re_order_status(function(rdata){
					if(rdata.status) {
						layer.closeAll();
						clearInterval(bt.soft.pub.wxpayTimeId);
						bt.msg({msg:"支付成功！",icon: 16, time: 0, shade: [0.3, "#000"]});
						bt.set_cookie('force',1);
						if(soft) soft.get_list();
						return;
					}
	...
	get_soft_list:function(p, type, callback){
		if(p == undefined) p=1;
		if(type == undefined) type = 0;
		...
		bt.send('get_soft_list','plugin/get_soft_list',{p:p,type:type,tojs:'soft.get_list',force:force},function(rdata){
			loading.close();
	...
	install_soft:function(item,version){ //安装单版本				
		...
		bt.confirm({msg:msg,title:item.type!=5?lan.soft.lib_install:lan.soft.install_title}, function() {
			...
			bt.send('install_plugin','plugin/install_plugin',{sName:item.name,version:version},function(rdata){
				loadT.close();		
	...
	update_soft:function(name,version,min_version){
		...
		bt.show_confirm('更新['+name+']','更新过程可能会导致服务中断,您真的现在就将['+name+']更新到['+version+'.'+min_version+']吗?',function(){
			var loadT = bt.load('正在更新['+name+'-'+version+'],请稍候...');
	...
	get_config_menu:function(name,version) //获取设置菜单显示
	{
		var menu = '';
		if(bt.os=='Linux'){
			menu = '<p onclick="bt.soft.get_tab_contents(\'config\',\''+name+'\')">'+lan.soft.config_edit+'</p><p onclick="bt.soft.get_tab_contents(\'change_version\',\''+name+'\',\''+version+'\')">'+lan.soft.nginx_version+'</p>';
			...
			switch(name)
			{
				...
				case 'nginx':
					...
					if(version != undefined || version !=''){
						 menu += '<p onclick="bt.soft.get_tab_contents(\'change_version\',\''+name+'\',\''+version+'\')">'+lan.soft.nginx_version+'</p>';
					}
		...
		switch(key)
		{
			...
			case 'change_version':
				...
				$(".btn-success").click(function(){
					...
					_this.install_soft(name,ver.split(" ")[1],5);
				});				
				break;
			...
			case 'phpmyadmin_php':
				bt.send('GetPHPVersion','site/GetPHPVersion',{},function(rdata){
					...
					$('.btn-success').click(function(){						
						var loadT = bt.load(lan.public.the);
						bt.send('setPHPMyAdmin','ajax/setPHPMyAdmin',{type: $("#phpversion").val()},function(rdata){
							loadT.close();
							bt.msg(rdata);
							if(rdata.status){
								setTimeout(function(){
				...
				function set_phpmyadmin(msg){
					...
					if(msg == 'close')
					{
						bt.confirm({msg:lan.soft.pma_pass_close},function(){
							var loading = bt.load(lan.public.the);
							bt.send('setPHPMyAdmin','ajax/setPHPMyAdmin',{type:msg,siteName:phpmyadmin},function(rdata){
								loading.close();
								bt.msg(rdata);
							})
						})
					...
					var loading = bt.load(lan.public.the);
					bt.send('setPHPMyAdmin','ajax/setPHPMyAdmin',{type:password_1,username:username,siteName:phpmyadmin},function(rdata){
						loading.close();
						bt.msg(rdata);
					})					
				}
			...
			case 'get_php_status':
				bt.soft.php.get_php_status(name,function(rdata){
					...
					$(".soft-man-con").html(con);
					bt.render_form('tab_php_status',arr);
				})
				break;
			case 'get_fpm_logs':
		...
		_this.get_soft_find(name,function(rdata){
			...
			{		
				...
				bodys = [	
					...
					'<p data-id="4" onclick="bt.soft.get_tab_contents(\'set_fpm_config\',\''+ver+'\')"  >'+lan.soft.php_main7+'</p>',
					'<p data-id="4" onclick="bt.soft.get_tab_contents(\'get_php_status\',\''+ver+'\')"  >'+lan.soft.php_main8+'</p>',
					'<p data-id="4" onclick="bt.soft.get_tab_contents(\'get_fpm_logs\',\''+ver+'\')"  >FPM'+lan.soft.log+'</p>',
					'<p data-id="4" onclick="bt.soft.get_tab_contents(\'get_slow_logs\',\''+ver+'\')"  >'+lan.public.slow_log+'</p>',
				...
				if(name=='phpmyadmin'){
					...
					menu = bt.soft.get_config_menu(name,rdata.version);	
				}				
			}
			layer.open({
				type: 1,
				...
				content: '<div class="bt-w-main" style="width:640px;">\
					<div class="bt-w-menu">\
						<p class="bgw" onclick="bt.soft.get_tab_contents(\'service\',\''+name+'\',\''+rdata.status+'\')">'+lan.soft.service+'</p>'
						+menu+
					'</div>\
...
bt.form ={
	...
	item:{
		data_access:{ title:'访问权限',items:[
					...
					$('#'+subid).remove();
					if(obj.val()=='ip'){
						obj.parent().append('<input id="'+subid+'" class="bt-input-text" type="text" name="address" placeholder="多个IP使用逗号(,)分隔" style="width: 203px; display: inline-block;">');
					}
				}
			}
		]},
...
bt.data = {
			...
			list:[
				{title:'域名',name:'webname',items:[
					{type:'textarea',width:'458px',callback:function(obj){
						...
						var res = ress.replace(new RegExp(/([-.])/g), '_');
						var ftp_user = res;
						var data_user = res;
						...
						var path = $('.path'+_bs).val();
						var dPath = bt.rtrim(bt.get_cookie('sites_path'),'/');
						if(path.substr(0,dPath.length)==dPath) $('.path'+_bs).val(dPath+'/'+res);	
						$('.ps'+_bs).val(ress);
				...
				{title:'根目录',name:'path',items:[
					{type:'text',width:'311px',value:bt.get_cookie('sites_path'),event: {css:'glyphicon-folder-open',callback:function(obj){bt.select_path(obj);}}}
				]},
				{title:'FTP',items:[
					{name:'ftp',type:'select',items:[
						...
						if(obj.val()=='true'){
							var _bs = obj.parents('div.bt-form').attr('data-id');
							var ftp_user = $('.webname'+_bs).data('ftp');
				...
				{title:'数据库',items:[
					{name:'sql',type:'select',items:[
						...
						{
							var _bs = obj.parents('div.bt-form').attr('data-id');
							var data_user = $('.webname'+_bs).data('database');
	...
	ftp:{
		add:{
			...
			list:[
				{title:'用户名',name:'ftp_username',callback:function(obj){
					var wootPath = bt.rtrim(bt.get_cookie('sites_path'),'/');
				...
				{title:'根目录',name:'path',value:bt.get_cookie('sites_path')}
			],
			btns:[
				{title:'关闭',name:'close'},
				{title:'提交',name:'submit',css:'btn-success',callback:function(rdata,load,callback){
		...
		set_port:{
			...
			area:'300px',
			list:[
				{title:'默认端口',name:'port',width:250}
			],
			btns:[
				{title:'关闭',name:'close'},
				{title:'提交',name:'submit',css:'btn-success',callback:function(rdata,load,callback){
		...
		set_password:{
			...
			list:[
				{title:'id',name:'id',hide:true},
				{title:'用户名',name:'ftp_username',width:'230px',disabled:true},
				{title:'密码',name:'new_password',items:[
					{type:'text',width:'230px',event: {css:'glyphicon-repeat',callback:function(obj){bt.refresh_pwd(16,obj);}}}
				]},
			],
			btns:[
				{title:'关闭',name:'close'},
