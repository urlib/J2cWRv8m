	...
	$.post('/files?action=GetDir',data,function(rdata){
		...
		if(rdata.DISK != undefined){
			...
			for(var i=0; i < rdata.DIR.length; i++){
				...
				Body +="<tr>"
						+"<td onclick=\"GetDiskList('"+rdata.PATH+"/"+rdata.DIR[i]+"')\" title='"+fmp[0]+"'><span class='glyphicon glyphicon-folder-open'></span>"+cnametext+"</td>"
						+"<td>"+getLocalTime(fmp[2])+"</td>"
						+"<td>"+fmp[3]+"</td>"
						+"<td>"+fmp[4]+"</td>"
						+"<td><span class='delfile-btn' onclick=\"NewDelFile('"+rdata.PATH+rdata.DIR[i]+"')\">X</span></td>"
					 +"</tr>";
			}
		...
		if(Path == '' && rdata.DIR != undefined){
			for(var i=0; i < rdata.DIR.length; i++){
				Body +="<tr><td onclick=\"GetDiskList('"+rdata.PATH+"/"+rdata.DIR[i]+"')\"><span class='glyphicon glyphicon-folder-open'></span>"+rdata.DIR[i]+"</td><td><span class='delfile-btn' onclick=\"NewDelFile('"+rdata.PATH+rdata.DIR[i]+"')\">X</span></td></tr>";
			}
			for(var i=0; i < rdata.FILES.length; i++){
