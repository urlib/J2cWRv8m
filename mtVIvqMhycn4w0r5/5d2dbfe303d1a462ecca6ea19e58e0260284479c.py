...
class ScanLogin(object):
    ...
    def set_login(self, get):       
        if os.path.exists(self.app_path+"login.pl"):
            ...
            os.remove(self.app_path+'login.pl')
...
class SelfModule():
    ...
    def __init__(self):
        ...
        print(self.user_info_file)
        if not os.path.exists(self.user_info_file):
            public.writeFile(self.user_info_file,"{}") 
    ...
    def blind_qrcode(self, get):
        ...
        version = '6.0.0'
        if os.path.exists('data/panel.version'): version = public.readFile('data/panel.version')

        qrcode_str = 'https://app.bt.cn/app.html?panel_url=' +  panel_addr+'&os=2&version='+version+'&panel_token=' + token + '?blind'
        return public.returnMsg(True, qrcode_str)
...
class wxapp(SelfModule, ScanLogin):
    ...
    def __init__(self):
        self.app_path = public.GetConfigValue('setup_path') + '/panel/data/'
        self.app_path_p = public.GetConfigValue('setup_path') + '/panel/plugin/app/'
    ...
    def _check(self, get):
        ...
        if hasattr(SelfModule, get['fun']):
            ...
            if get['sgin'] == public.md5(binascii.hexlify(base64.b64encode(encryption_str.encode('utf-8'))).decode('utf-8')):
    ...
    def blind(self, get):
        ...
        public.writeFile(self.app_path+"user.json", json.dumps(self.user_info))
        public.writeFile(self.app_path_p + "user.json", json.dumps(self.user_info))
        path1 = "%s/token.pl" % self.app_path
        path2 = "%s/token.pl" % self.app_path_p
