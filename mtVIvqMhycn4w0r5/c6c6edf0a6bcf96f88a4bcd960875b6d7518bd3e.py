...
class data:
    ...
    '''
    def setPs(self,get):        
        if not hasattr(get, 'id'): public.returnMsg(False,'缺少必要参数网站ID，请重新操作.');
    ...
    def GetField(self,tableName):
        fields = {
            'sites'     :   "id,name,path,status,ps,addtime,edate,type",
            'ftps'      :   "id,pid,name,password,status,ps,addtime,path",
            'databases' :   "id,pid,name,username,password,accept,ps,addtime,type",
